...
app = Flask(__name__, static_folder=str(ROOT / "views" / "static"), static_url_path="/static")
...
def _last_tr_sync_path(user_id: str) -> Path:
    return DATA_DIR / "users" / user_id / "last_tr_sync.json"


def _load_last_tr_sync(user_id: str) -> Optional[str]:
    """
    Retourne le timestamp ISO de l'event le plus recent synchronise via TR API.
    None si premiere sync (ou fichier corrompu).

    Volontairement non chiffre : un timestamp n'est pas un secret.
    """
    path = _last_tr_sync_path(user_id)
    if not path.exists():
        return None
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
...
def _save_last_tr_sync(user_id: str, last_event_iso: str) -> None:
    """Persiste le timestamp pour la prochaine sync incrementale."""
    path = _last_tr_sync_path(user_id)
...
def api_parse():
    ...
    try:
        fichier.save(tmp)
        # Utiliser le nouveau parseur MVC
        parser = PDFTradeRepublicParser()
        transactions = parser.extract_csv(tmp)
    finally:
...
@app.route("/api/tr/sync/init", methods=["POST"])
@require_pin
def api_tr_sync_init():
    """
    ETAPE 1 - Initiate login Trade Republic.

    Body JSON : {"phone": "+33...", "pin": "1234"}
    Reponse JSON : {"ok": true, "session_id": "...", "countdown_seconds": N}

    Apres cet appel, l'app TR mobile recoit un push avec un code de validation.
    Le code est ensuite envoye via /api/tr/sync/verify.

    SECURITE :
    - Aucune persistance des creds TR
    - L'instance pytr est conservee en RAM serveur (TTL 5min) pour permettre
      la 2eme etape avec le code recu sur l'app mobile
    """
    ...
    try:
        ...
        logger.warning("[/api/tr/sync/init] payload invalide : %s", e)
    ...
    try:
        ...
        logger.error("[/api/tr/sync/init] initiate KO : %s", e)
        return jsonify({"error": str(e)}), 502
    except Exception as e:
        logger.error("[/api/tr/sync/init] exception inattendue : %s", type(e).__name__)
        return jsonify({"error": "Erreur interne pendant l'initialisation"}), 500
    finally:
        tr_creds = None
        del tr_creds

    # Stockage RAM de l'instance pytr (TTL geree par le store)
    session_id = TRSessionStore().create(api=init_result.api, user_id=user_id)

    # Mode incremental ou full ?
    last_event_iso = _load_last_tr_sync(user_id)
    ...
    return jsonify({
        ...
        "sync_mode": sync_mode,
        "last_event_iso": last_event_iso,  # juste informatif, pas un secret
        "message": (
            f"Code envoye sur l'app TR. Tu as {init_result.countdown_seconds}s "
            f"pour le saisir."
        ),
    })


@app.route("/api/tr/sync/verify", methods=["POST"])
@require_pin
def api_tr_sync_verify():
    """
    ETAPE 2 - Valide le code et lance la synchronisation.

    Body JSON : {"session_id": "...", "code": "1234"}
    Reponse JSON : {"ok": true, "added": N, "skipped": N, "total": N}

    SECURITE :
    - Le session_id n'est pas logge
    - L'instance pytr est detruite apres usage (succes OU echec)
    - En cas d'echec, on re-essaie pas : il faut refaire /init
    """
    from services.tr_sync_service import TRSyncService
    from services.tr_session_store import TRSessionStore
    ...
    user_id = _get_session_user()
    session_pin = _get_session_pin()
    if not user_id or not session_pin:
        return jsonify({"error": "Authentification requise.", "auth_required": True}), 401

    # Recup l'instance pytr depuis le store
    store = TRSessionStore()
    try:
        api = store.get(session_id, expected_user_id=user_id)
    except KeyError:
        return jsonify({
            "error": "Session expiree ou invalide. Recommence depuis l'etape 1.",
        }), 410  # Gone

    # Decrypt CSV utilisateur dans un tmp en clair
    fd_csv, path_csv = tempfile.mkstemp(suffix=".csv")
    os.close(fd_csv)
    tmp_csv = Path(path_csv)
    ...
    try:
        releve = _releve_path(user_id)
        user_known = _load_user_known_merchants(user_id)
        # Sync incrementale : on ne re-pull que les events plus recents que la derniere fois
        last_event_iso = _load_last_tr_sync(user_id)

        if releve.exists():
            tmp_csv.write_text(_read_releve_csv_text(releve, session_pin), encoding="utf-8")
        else:
            tmp_csv.unlink(missing_ok=True)
            tmp_csv.touch()
        ...
        try:
            ...
            logger.error("[/api/tr/sync/verify] sync KO : %s", e)
            return jsonify({"error": str(e)}), 502
        except Exception as e:
            logger.error("[/api/tr/sync/verify] exception inattendue : %s", type(e).__name__)
            return jsonify({"error": "Erreur interne pendant la synchronisation"}), 500

        # Re-encrypt et ecrit dans le storage utilisateur
        _write_releve_csv_text(releve, tmp_csv.read_text(encoding="utf-8"), session_pin)

        # Persiste le timestamp pour la prochaine sync incrementale
        # (uniquement si on a recu au moins un event)
        if result.latest_event_iso:
            _save_last_tr_sync(user_id, result.latest_event_iso)

    finally:
        tmp_csv.unlink(missing_ok=True)
        # IMPORTANT : on detruit l'instance pytr du store, qu'on ait reussi ou non
        store.delete(session_id)
        # Liberation explicite de la reference locale a api
        api = None
        del api
    ...
    return jsonify({
        ...
        "sync_mode": mode,
        "message": (
            f"Sync {mode} : +{result.added} transaction(s) ajoutee(s), "
            f"{result.skipped} doublon(s) ignore(s) — "
            f"{result.total} ligne(s) au total."
        ),
    })
...
def api_info():
    ...
    return jsonify({
        "app": "Check Trade",
        "version": "2.1-MVC-Hybrid",
        "parser": "PDFTradeRepublicParser",
        "routes_available": len(list(app.url_map.iter_rules())),
        "features": [
            "Classification automatique des marchands",
            "Categorisation intelligente", 
            "Architecture MVC avec parseur abstrait"
        ]
    })
